const DROPDOWN = {
    ...
    <div v-show="isShow" 
        class="absolute bg-white border z-10"
        :class="isDown ? 'bottom-8 left-0' : 'top-15 left-0'"
        >
        <div v-if="isStatus === 'Archive'">
            <button @click="$emit('onRestore', id )" class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-else>
            <div v-if="permission === 'manager'">
                <div v-if="isStatus === 'Stopped'">
                    <button @click="$emit('onStart', id )" class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
                ...
                <div v-else-if="isStatus === 'Running'">
                    <button @click="$emit('onStop', id )" class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
                ...
                <button @click="$emit('onSuspend', id )" class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M13.477 14.89A6 6 0 015.11 6.524l8.367 8.368zm1.414-1.414L6.524 5.11a6 6 0 018.367 8.367zM18 10a8 8 0 11-16 0 8 8 0 0116 0z" clip-rule="evenodd" />
                    </svg>
                    <span class="text-sm">Suspend</span>
                </button>
                <button class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none">
                ...
                <button @click="$emit('onArchive', id )" class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <button @click="$emit('onDelete', id )" class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div v-else-if="permission === 'supervisor'">
                <button class="w-full flex flex-row space-x-5 hover:bg-gray-100 focus:outline-none px-3 py-2">
