...
// donut chart init function
function chartInit(chart_div) {
	let
		data = chart_div.getAttribute('data').split('/'),
		value = Math.floor((data[0] / data[1]) * 100) / 100;
	...
	if (data.length == 2) {
		...
		new DonutChart(chart_div, {
			r: chart_div.offsetWidth / 2,
			stroke: 20,
			scale: 150,
			items: [
				{ label: "used", value }
...
// all donut chart must init when DOM is ready and screen is resize
let chart_divs = document.querySelectorAll(".chart");
if (chart_divs) {
	function chartInit(chart_div) {
		let
			data = chart_div.getAttribute('data').split('/'),
	...
	chart_divs.forEach(chart_div => {
		...
		window.addEventListener('resize', () => {
			chart_div.querySelector('svg').remove();
			chartInit(chart_div);
		});
	});

	// sample code incase chart may update and DonutChart obj must stort global
	// mychart.update({
	//     items: [
	//         { label: "J", value: .1 },
	//         { label: "Q", value: .3 },
